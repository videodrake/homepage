            ...
            if (methods.aSubCategory[iCateNo] != undefined) {
                ...
                $(methods.aSubCategory[iCateNo]).each(function() {
                    ...
                    if (methods.aSubCategory[sNextParentNo] == undefined) {
                        ...
                        var sHref = '/product/detail.html?product_no=11';
                    } else {
                        aHtml.push('<li id="cate'+this.cate_no+'">');
                        var sHref = '#none';
                    }
                    aHtml.push('<a href="/product/detail.html?product_no=11" class="view" cate="'+this.param+'" data-i18n="LIST.PRD_CATE_NO_'+this.cate_no+'" data-i18n-new>'+this.name+'</a>');
                    if (methods.aSubCategory[sNextParentNo] != undefined)  aHtml.push('<a href="'+sHref+'" onclick="subMenuEvent(this);" class="cate">상품보기</a>');
                        ...
                        $(methods.aSubCategory[sNextParentNo]).each(function() {
                            ...
                            if (methods.aSubCategory[sNextParentNo2] != undefined) {
                                ...
                                $(methods.aSubCategory[sNextParentNo2]).each(function() {
                                    aHtml.push('<li class="noChild" id="cate'+this.cate_no+'">');
                                    var sCateSelected = (checkInArray(sMyCateList, this.cate_no) == true) ? ' selected' : '';
                                    aHtml.push('<a href="/product/detail.html?product_no=11" class="view" cate="'+this.param+'" onclick="subMenuEvent(this);" data-i18n="LIST.PRD_CATE_NO_'+this.cate_no+'" data-i18n-new>'+this.name+'</a>');
                                    aHtml.push('</li>');
                                });
                                aHtml.push('</ul>');
                            }
